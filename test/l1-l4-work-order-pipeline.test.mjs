...
} from "../scripts/lib/l1-l4-work-order-pipeline.mjs";
...
function draftFor(sourceId, { highRisk = false } = {}) {
  const concreteTasks = [
    `In scripts/lib/l2-l3-selection-audit.mjs, check source_id=${sourceId}; expected result is L3 forwards only valid handoff rows.`,
    `In test/l2-l3-selection-audit.test.mjs, check pool assignment for ${sourceId}; expected result is green and review-only.`,
    `In scripts/lib/l4-work-order-review.mjs, check handoff target for ${sourceId}; expected result is no work-order execution.`,
    `In test/l4-work-order-review.test.mjs, check authorization fields for ${sourceId}; expected result is recommendation-only metadata.`
  ];
  if (highRisk) {
...
test("L1-L4 pipeline CLI can run from an existing L2 report without LLM calls", async () => {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(process.execPath, [
      "scripts/l1-l4-work-order-pipeline.mjs",
      "--l2-report",
      l2Path,
      "--out-dir",
      tempRoot,
