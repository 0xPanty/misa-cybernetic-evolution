...
} from "../scripts/lib/l2-gate-intercept-analysis.mjs";
...
function nearPassDraft(sourceId) {
  return {
    ...
    evidence_refs: [`ref:${sourceId}`, "runs:test-fixture"],
    concrete_tasks: [
      `In scripts/lib/external-trajectory-online-shadow-contract.mjs, check source_id=${sourceId} field=authority; expected result is suggestion_only.`,
      `In test/external-trajectory-online-shadow-contract.test.mjs, check signal=public_memory_boundary for ${sourceId}; expected result is no route change.`,
...
test("L2 gate intercept analysis writes local artifacts and CLI output", async () => {
  ...
  const { stdout } = await execFileAsync(process.execPath, [
    path.join(process.cwd(), "scripts", "l2-gate-intercept-analysis.mjs"),
    "--runs-dir",
    "runs",
    "--dry-run"
  ], { cwd: repoRoot });
