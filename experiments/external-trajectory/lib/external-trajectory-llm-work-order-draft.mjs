...
function commandForPerceptionDigest(perceptionDigestPath) {
  ...
  return `npm run experiments:external:online-shadow -- --json --dry-run --perception-digest ${perceptionDigestPath}`;
}
...
function contextForPacket(packet) {
  ...
  const commonAnchors = [
    ...
    "draft_no_write",
    "experiments:external:online-shadow",
...
function allowedCommandsForPacket(packet, perceptionDigestPath) {
  ...
  if (sourceClass === "public_boundary") {
    ...
    commands.push("node --test test/ci-workflow.test.mjs experiments/external-trajectory/test/external-trajectory-online-shadow-contract.test.mjs");
  }
