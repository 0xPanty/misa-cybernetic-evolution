...
import { buildExternalTrajectoryOnlineShadowContractReport } from "../lib/external-trajectory-online-shadow-contract.mjs";
import {
...
test("LLM work-order packets include context anchors and command whitelist", () => {
  ...
  assert.ok(packets[0].allowed_verification_commands.some((command) => command.startsWith("npm run experiments:external:online-shadow")));
});
...
test("LLM work-order gate treats mandatory human-in-the-loop as an explicit expectation", () => {
  ...
  const draft = {
    ...
    evidence_refs: [...packet.workOrder.evidence_refs],
    concrete_tasks: [
      "In experiments/external-trajectory/lib/external-trajectory-online-shadow-contract.mjs, check source_id=shadow-public-memory-risk-001 field=execution_policy.route_change_allowed; expected result is false and winner_change_allowed=false.",
...
test("LLM work-order gate recognizes project-specific state labels as expectations", () => {
  ...
  const draft = {
    ...
    evidence_refs: [...packet.workOrder.evidence_refs],
    concrete_tasks: [
      "In experiments/external-trajectory/lib/external-trajectory-online-shadow-contract.mjs, check source_id shadow-public-memory-risk-001 for the publication_allowed field; expected result is false.",
      "In experiments/external-trajectory/test/external-trajectory-online-shadow-contract.test.mjs, check the farcaster_public_memory_risk signal; expected result is a draft_no_write status with no state mutation.",
...
test("LLM work-order gate recognizes concrete no-effect expectation wording", () => {
  ...
  const draft = {
    ...
    concrete_tasks: [
      ...
      `In experiments/external-trajectory/test/external-trajectory-online-shadow-contract.test.mjs, check l2_candidate_mode and candidate_count for ${packet.source_id}; expected result is enforcement of a single candidate count in the shadow contract.`
    ],
    acceptance_criteria: [
...
test("LLM work-order CLI candidate-recheck switch requests two candidates", async () => {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(process.execPath, [
      "experiments/external-trajectory/scripts/external-trajectory-llm-work-order-draft.mjs",
