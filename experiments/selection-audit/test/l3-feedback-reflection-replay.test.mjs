...
test("real historical replay finds the known cheap-route cluster without false positives", async () => {
  ...
  try {
    ...
    assert.ok(result.summary.baseline_feedback_count >= 3);
    assert.ok(result.summary.baseline_bad_caught_count >= 3);
    assert.ok(result.summary.candidate_trigger_count >= 3);
    assert.ok(result.summary.candidate_bad_caught_count >= 3);
    assert.equal(result.summary.candidate_good_false_positive_count, 0);
    assert.ok(result.summary.newly_caught_count >= 0);
    assert.ok(result.summary.current_vs_candidate_gain >= 0);
    assert.ok(result.summary.baseline_primary_agent_review_suggested_count >= 2);
    assert.ok(result.summary.candidate_primary_agent_review_suggested_count >= 2);
...
test("reflection replay CLI emits the expected summary", async () => {
  ...
  try {
    const { stdout } = await execFileAsync(process.execPath, [
      "experiments/selection-audit/scripts/l3-feedback-reflection-replay.mjs",
      "--runs-dir",
      "runs",
      "--out-dir",
