...
```text
Do route, repair-ticket, work-order, retrieval, tournament, and judge-escalation
...
- signal-layer coverage stays explicit: source signals, Qianxuesen route
  signals, shadow perception hints, work-order pressure, retrieval ranking
  inputs, and tournament quality signals;
...
first, work-order signals decide who should inspect it, retrieval signals decide
what comes back first, and tournament signals compare safe local drafts. Only
